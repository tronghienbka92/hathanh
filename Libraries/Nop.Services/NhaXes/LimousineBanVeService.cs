...
namespace Nop.Services.NhaXes
{
    public class LimousineBanVeService : ILimousineBanVeService
    {
        ...
        {
            var query = _datveRepository.Table.Where(c =>c.KhachHangId == KhachHangId && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
            if (HanhTrinhId > 0)
                query = query.Where(c => c.HanhTrinhId == HanhTrinhId);
            query = query.OrderByDescending(c => c.NgayDi).ThenByDescending(c => c.Id);
            return query.FirstOrDefault();
        }
        ...
        public virtual List<ChuyenDi> GetAllChuyenDiTrongNgay(int NhaXeId, int HanhTrinhId, DateTime NgayDi)
        {
            NgayDi = NgayDi.Date;
            var query = _chuyendiRepository.Table.Where(c => c.NhaXeId == NhaXeId && c.TrangThaiId != (int)ENTrangThaiXeXuatBen.HUY
                && c.HanhTrinhId == HanhTrinhId
        ...
        public virtual ListDoanhThuTongHop GetDoanhThuTongHopTheoNgay(DateTime NgayDi, int NhaXeId)
        {
            ...
            var HNHanhtrinhids = _hanhtrinhkhuvucRepository.Table.Where(c => c.KhuVucId == (int)ENKhuVuc.HA_NOI).Select(c => c.HanhTrinhId);
            ...
            var chuyendiHN = query.Where(c => HNHanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();
            var chuyendiQN = query.Where(c => QNHanhtrinhids.Contains(c.HanhTrinhId)).Select(c => c.Id).ToArray();

            //doanh thu ha noi
            item.DTHaNoi = new DoanhThuTongHop();
            var datveshn = _datveRepository.Table.Where(c => chuyendiHN.Contains(c.ChuyenDiId.Value) && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
            item.DTHaNoi.TongLuot = datveshn.Select(c => c.ChuyenDiId).Distinct().Count();
            item.DTHaNoi.SoKhach = datveshn.Count();
            item.DTHaNoi.ThanhTien = 0;
            if (datveshn.Count() > 0)
            {
                item.DTHaNoi.ThanhTien = datveshn.Sum(c => c.GiaTien);
            }
            //doanh thu quang ninh
            item.DTQuangNinh = new DoanhThuTongHop();
            var datvesqn = _datveRepository.Table.Where(c => chuyendiHN.Contains(c.ChuyenDiId.Value) && (c.TrangThaiId == (int)ENTrangThaiDatVe.DA_DI || c.TrangThaiId == (int)ENTrangThaiDatVe.DA_XEP_CHO));
            item.DTQuangNinh.TongLuot = datvesqn.Select(c => c.ChuyenDiId).Distinct().Count();
            item.DTQuangNinh.SoKhach = datvesqn.Count();
            item.DTQuangNinh.ThanhTien = 0;
            if (datvesqn.Count() > 0)
            {
                item.DTHaNoi.ThanhTien = datvesqn.Sum(c => c.GiaTien);
            }
        ...
        public virtual HanhTrinhLoaiXeGiaVe GetHanhTrinhLoaiXeGiaVe(int HanhTrinhId, int LoaiXeId, int SoDoGheQuyTacId)
        {
            return _hanhtrinhloaixeRepository.Table.Where(c => c.HanhTrinhId == HanhTrinhId && c.LoaiXeId == LoaiXeId && c.SoDoGheId == SoDoGheQuyTacId).FirstOrDefault();
        }
