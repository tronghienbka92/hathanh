...
namespace Nop.Services.NhaXes
{
    public partial class NhaXeService : INhaXeService
    {
        ...
        private readonly IRepository<DiaChiMac> _diachimacRepository;
        ...
        public virtual List<ChuyenDi> GetAllXeXuatBenByNgayDi(int NhaXeId, DateTime NgayDi)
        {
            var query = _chuyendiRepository.Table.Where(m => m.NhaXeId == NhaXeId && m.xevanchuyen !=null && m.TrangThaiId != (int)ENTrangThaiXeXuatBen.HUY
               && m.NgayDi.Year >= NgayDi.Year
               && m.NgayDi.Month >= NgayDi.Month
            ...
            if (ThangId > 0 && NamId > 0)
            {
                ...
                .Where(c => (c.TrangThaiId == (int)ENTrangThaiXeXuatBen.DANG_DI
                    ...
                    ).ToList().Select(m =>
                    {
                        ...
                        item.Thang = ThangId;
                        item.Nam = NamId;
                        item.BienSo = m.xevanchuyen.BienSo;
                        item.SoLuot = 1;
                        return item;
                    }).ToList();
                return query;
        ...
        #endregion
        #region cau hinh dia chi mac voi line dien thoai
        public List<DiaChiMac> GetAllDiaChiMac()
        {
            return _diachimacRepository.Table.ToList();
        }
        #endregion
    }
}
