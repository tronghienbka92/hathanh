...
namespace Nop.Services.ExportImport
{
    ...
    public partial class ExportManager : IExportManager
    {
        ...
        public virtual void ExportLenhPhuToXlsx(Stream stream, List<DatVe> hanhkhachs, DateTime NgayDi, string BienSo,ChuyenDi ChuyenDi)
        {
            if (stream == null)
                throw new ArgumentNullException("stream");
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                worksheet.Cells[1, 1].Value = "CÔNG TY TNHH BẤT ĐỘNG SẢN VÀ DU LỊCH VÂN ĐỒN";

                worksheet.Cells["A2:D2"].Merge = true;
                worksheet.Cells[2, 1].Value = "Địa chỉ: Số 183, Đường 334, Khu 5, Thị Trấn Cái Rồng, Huyện Vân Đồn, Tỉnh Quảng Ninh";

                worksheet.Cells["A3:D3"].Merge = true;
                worksheet.Cells[3, 1].Value = "Tel: 024.3200.4499 / 024.32032.444 / 02033.991.123";

                worksheet.Cells["B4"].Value = "Chuyến: " + ChuyenDi.hanhtrinh.MaHanhTrinh;

                worksheet.Cells["C4"].Value = "Giờ: " + ChuyenDi.NgayDi.ToString("HH:ss");

                worksheet.Cells["D4"].Value = "Ngày: " + ChuyenDi.NgayDi.ToString("dd-MM-yyyy");

                string laixe = "";
                if (ChuyenDi.laixe != null)
                    laixe = ChuyenDi.laixe.HoVaTen;
                worksheet.Cells["E4"].Value ="Lái xe: " + laixe;

                string bienso = "";
                if (ChuyenDi.xevanchuyen != null)
                    bienso = ChuyenDi.xevanchuyen.BienSo;
                worksheet.Cells["F4"].Value = "Biển số: " + BienSo;

                worksheet.Cells["C5:D5"].Merge = true;
                worksheet.Cells["C3"].Value = "DANH SÁCH KHÁCH HÀNG";
                ...
                worksheet.Cells["B6"].Value = "Số điện thoại";

                worksheet.Cells["C6:C7"].Merge = true;
                worksheet.Cells["C6"].Value = "Tên hành khách";

                worksheet.Cells["D6:D7"].Merge = true;
                worksheet.Cells["D6"].Value = "Điểm đón";

                worksheet.Cells["E6:E7"].Merge = true;
                worksheet.Cells["E6"].Value = "Điểm trả";

                worksheet.Cells["F6:F7"].Merge = true;
                worksheet.Cells["F6"].Value = "Ghế ngồi";

                worksheet.Cells["G6:G7"].Merge = true;
                worksheet.Cells["G6"].Value = "Số tiền";
                ...
                foreach (var item in hanhkhachs)
                {
                    ...
                    string tenkh = item.khachhang.Ten;
                    worksheet.Cells[row, col].Value = tenkh;
                    col++;

                    worksheet.Cells[row, col].Value = item.TenDiemDon;
                    col++;


                    worksheet.Cells[row, col].Value = item.TenDiemTra;
                    col++;

                    worksheet.Cells[row, col].Value = item.sodoghe.Val;
                    col++;

                    worksheet.Cells[row, col].Value = item.GiaTien;
                ...
                //style body
                var modelRows = row;
                string modelRange = "A7:G" + modelRows.ToString();
                var modelTable = worksheet.Cells[modelRange];
                // Assign borders
                modelTable.Style.Border.Top.Style = ExcelBorderStyle.Dashed;
                modelTable.Style.Border.Left.Style = ExcelBorderStyle.Thin;
        ...
        public virtual void ExportDTTongHopToXlsx(Stream stream, List<ListDoanhThuTongHop> DTTongHop, DateTime TuNgay, DateTime DenNgay)
        {
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                worksheet.Cells["B4"].Value = "Ninh Bình ";

                worksheet.Cells["E4:G4"].Merge = true;
                worksheet.Cells["E4"].Value = "Thái Bình";

                worksheet.Cells["H4:J4"].Merge = true;
                worksheet.Cells["H4"].Value = "Nam Định";
                ...
                foreach (var item in DTTongHop)
                {
                    ...
                    worksheet.Cells[row, col].Value = item.DTHaNoi.TongLuot;
                    col++;
                    worksheet.Cells[row, col].Value = item.DTHaNoi.SoKhach;
                    col++;
                    worksheet.Cells[row, col].Value = item.DTHaNoi.ThanhTien.ToString("###,###");
                    col++;
                    ...
                    decimal Tong = item.DTHaNoi.ThanhTien + item.DTQuangNinh.ThanhTien;
                ...
                worksheet.Cells[row, coltong].Value = DTTongHop.Sum(c => c.DTHaNoi.TongLuot);
                coltong++;
                worksheet.Cells[row, coltong].Value = DTTongHop.Sum(c => c.DTHaNoi.SoKhach);
                coltong++;
                worksheet.Cells[row, coltong].Value = DTTongHop.Sum(c => c.DTHaNoi.ThanhTien).ToString("###,###");
                coltong++;
